    ...
    for (let x = 0; x < BOARD_W; x += 1) {
        for (let y = 0; y < BOARD_H; y += 1) {
            drawable_chestgrass.push({ ...DEFAULT_DRAWABLE_IMG, image: get_asset('grass'), src: src_box,
                dst: {
                    ...board.chests[y][x].sprite.dst,
                    h: board.chests[y][x].sprite.dst.h * 0.3,
                    y: (board.chests[y][x].sprite.dst.y + (board.chests[y][x].sprite.dst.h - board.chests[y][x].sprite.dst.h * 0.27))
                } });
            anim.push({
                frame: 0,
                max_frame: 18,
...
export const animate_grass = () => {
    for (let single of anim) {
        single.frame += 0.085;
        single.object.x = single.object.w * (single.frame | 0);

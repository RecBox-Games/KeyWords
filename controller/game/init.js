...
// }
const construct_chest = (x, y, box, role) => {
...
const construct_Board = (role) => {
    ...
    for (y = 0; y < BOARD_H; y += 1) {
        ...
        for (x = 0; x < BOARD_W; x += 1) {
            let newChest = construct_chest(x, y, boundingBox, role);
            let newButton = new Button({ ...boundingBox }, undefined, (role == GUESSER ? undefined : chest_clicked_giver), (role == GUESSER ? chest_clicked_guessser : () => { get_board().showOverlay = false; }));
...
const construct_overlay_guesser = () => {
    const ctx = get_context();
    const new_Overlay = {
...
export const init_main_screen = (role) => {
    board = {
        buttons: [],
        chests: [],

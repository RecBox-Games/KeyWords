...
export const init_menu = () => {
    menu = {
        ...
        blueTeam: {
            ...
            guesserBtn: new Button({ x: 0, y: 0, h: 0, w: 0 }, undefined, undefined, (self) => { menu.team = BLUE_TEAM; menu.role = GUESS; set_chosen_text(); self._active = false; }),
            giverBtn: new Button({ x: 0, y: 0, h: 0, w: 0 }, undefined, undefined, (self) => { menu.team = BLUE_TEAM; menu.role = CLUE; set_chosen_text(); self._active = false; }),
        ...
        redTeam: {
            ...
            guesserBtn: new Button({ x: 0, y: 0, h: 0, w: 0 }, undefined, undefined, (self) => { menu.team = RED_TEAM; menu.role = GUESS; set_chosen_text(); self._active = false; }),
            giverBtn: new Button({ x: 0, y: 0, h: 0, w: 0 }, undefined, undefined, (self) => { menu.team = RED_TEAM; menu.role = CLUE; set_chosen_text(); self._active = false; }),
            guesserSprite: { ...DEFAULT_DRAWABLE_TEXT, text: "Guesser" },
            giverSprite: { ...DEFAULT_DRAWABLE_TEXT, text: "Clue giver" },
            giverRect: DEFAULT_DRAWABLE_RECT,
        },
...
const set_wait_text = (ctx) => {
    menu.text.text = "Waiting for game to start...";
    ctx.ctx.font = menu.text.font;
    let textDim = ctx.ctx.measureText(menu.text.text);
    let newTextBox = scale_and_center({ x: 0, y: 0, w: textDim.width, h: textDim.actualBoundingBoxAscent + textDim.actualBoundingBoxDescent }, menu.container.boundingBox, 1);
...
const resize_team = (box, team, ctx) => {
    ctx.ctx.font = team.name.font;
    let textDim = ctx.ctx.measureText(team.name.text);
    let newTextBox = scale_and_center({ x: 0, y: 0, w: textDim.width, h: textDim.actualBoundingBoxAscent + textDim.actualBoundingBoxDescent }, box, 1);
    ...
    newTextBox = scale_and_center({ x: 0, y: 0, w: textDim.width, h: textDim.actualBoundingBoxAscent + textDim.actualBoundingBoxDescent }, box, 1);
